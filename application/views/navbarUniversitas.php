...
<body>
    ...
    <div class="container-fluid row gap-5 p-5 border">
            ...
            <div id="universitas" class="col p-3">
                <h3 class="mx-3 mb-5">Universitas</h3>
                <?php $counter = 0; ?>
                <?php foreach ($universitas as $univ): 
                    $limitedContent = substr($univ->news_content, 0, 250); // Limit content to 200 characters
                ?>
                    <div class="terkini-item d-flex mb-4">
                        <div class="mx-3">
                            <img width="400" class="rounded-4" src="<?= base_url('assets/dbimages/' . $univ->news_images) ?>" alt="terkini Image">
                        </div>
                        <div class="content-univ mx-5">
                            <p style="color: #fe7d21;" class="content-section-2 fs-5 lh-lg">
                            <?= date('d F Y', strtotime($univ->news_publish)); ?>
                            </p>
                            <a href="<?php echo base_url('newsView/newsDetail/'.$univ->news_code); ?>" class="fs-2 headline-section-2 text-wrap">
                                <?= $univ->news_headline ?>
                            </a>
                            <p style="text-align:justify;" class="content-section-2 text-wrap fs-5 lh-lg">
                                <?= $limitedContent . (strlen($univ->news_content) > 5 ? ' ... ' : ''); ?>
                            </p>
                            
                            <a class="selengkapnya text-end fs-5 my-3 py-2 px-3 rounded-3" href="<?php echo base_url('newsView/newsDetail/'.$univ->news_code); ?>">
                                Selengkapnya > 
                            </a> 
