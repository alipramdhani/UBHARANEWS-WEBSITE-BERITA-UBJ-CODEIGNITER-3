    ...
    <section id="section-2">
        <div class="container-fluid row gap-5 p-5 border">
            ...
            <div id="olahraga" class="col p-3">
                <h3 class="mx-3 mb-5">Olahraga</h3>
                <?php $counter = 0; ?>
                <?php foreach ($olahragas as $sports): 
                    $limitedContent = substr($sports->news_content, 0, 250); // Limit content to 200 characters
                ?>
                    <div class="terkini-item d-flex mb-4">
                        <div class="mx-3">
                            <img width="400" class="rounded-4" src="<?= base_url('assets/dbimages/' . $sports->news_images) ?>" alt="terkini Image">
                        </div>
                        <div class="content-sports mx-5">
                            <p style="color: #fe7d21;" class="content-section-2 fs-5 lh-lg">
                            <?= date('d F Y', strtotime($sports->news_publish)); ?>
                            </p>
                            <a href="<?php echo base_url('newsView/newsDetail/'.$sports->news_code); ?>" class="fs-2 headline-section-2 text-wrap">
                                <?= $sports->news_headline ?>
                            </a>
                            <p style="text-align:justify;" class="content-section-2 text-wrap fs-5 lh-lg">
                                <?= $limitedContent . (strlen($sports->news_content) > 5 ? ' ... ' : ''); ?>
                            </p>
                            
                            <a class="selengkapnya text-end fs-5 my-3 py-2 px-3 rounded-3" href="<?php echo base_url('newsView/newsDetail/'.$sports->news_code); ?>">
                                Selengkapnya > 
                            </a> 
